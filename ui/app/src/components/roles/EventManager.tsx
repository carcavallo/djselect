...
import { PencilSquareIcon, ClipboardDocumentCheckIcon, TrashIcon } from '@heroicons/react/24/outline';
...
const EventManager: React.FC = () => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await fetch(`http://localhost/api/usevents/${user?.user_id}`, {
        method: 'GET',
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <ClipboardDocumentCheckIcon 
                    className="h-6 w-6 text-gray-400 cursor-pointer" 
                    onClick={() => navigate(`/event-requests/${event.event_id}`)} 
                    aria-hidden="true" 
                  />
                  <TrashIcon
                    className="h-6 w-6 text-gray-400 cursor-pointer"
